...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log('[👢] RemoveParticipant response:', text)

    if (!res.ok) {
      return NextResponse.json({ error: 'Failed to remove participant', detail: text }, { status: res.status })
    }
